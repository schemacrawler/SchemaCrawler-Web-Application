...
public class DiagramRequestUtility {
  ...
  private static final ObjectMapper OBJECT_MAPPER = newConfiguredObjectMapper();
  ...
  String diagramRequestToJson(final DiagramRequest diagramRequest) {
    ...
    try {
      return OBJECT_MAPPER.writeValueAsString(diagramRequest);
  ...
  DiagramRequest readDiagramRequest(final Reader diagramRequestReader) {
    ...
    try {
      return OBJECT_MAPPER.readValue(diagramRequestReader, DiagramRequest.class);
    } catch (final JacksonException e) {
