...
- Start application from the jar file, run 
  `java -DAWS_ACCESS_KEY_ID=xxxxx -DAWS_SECRET=xxxxx -DAWS_S3_BUCKET=xxxxx -jar target/schemacrawler-webapp-16.16.3.7.jar`
  or, in local mode,
  `java -Dspring.profiles.active=local -jar target/schemacrawler-webapp-16.16.3.7.jar`
...
You can use the [SchemaCrawler web application as an API](https://app.swaggerhub.com/apis/sualeh/schemacrawler-web-application/16.16.3.7). 
...
curl \
  ...
  http://localhost:8080/schemacrawler
```
...
```sh
curl http://localhost:8080/schemacrawler/results/l0nk0wu4t2a3/diagram --output diagram.png
```
